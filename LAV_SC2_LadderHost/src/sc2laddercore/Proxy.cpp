...
{
    LavNoopHumanAgent humanAgent;
    sc2::Coordinator coordinator;
    coordinator.SetParticipants({
        sc2::CreateParticipant(m_botConfig.Race, &humanAgent),
    });
    PrintThread{} << "Starting human API bridge for " << m_botConfig.BotName << " on port " << gamePort << std::endl;
    coordinator.Connect(gamePort);
    coordinator.SetupPorts(2, startPort, false);
    coordinator.JoinGame();
    coordinator.SetTimeoutMS(10000);
    PrintThread{} << "Human API bridge joined game for " << m_botConfig.BotName << std::endl;
